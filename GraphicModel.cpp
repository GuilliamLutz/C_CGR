...
*/
GraphicModel::GraphicModel(const std::string& fileName)
{
...
void GraphicModel::loadMaterialFile(const std::string& fileName, const std::string& dirName)
{
	std::cout << "Loading " << fileName << std::endl;
	std::ifstream pFile(fileName.c_str());
	if (!pFile.is_open())
...
GraphicModel::MaterialInfo::MaterialInfo()
{
	texture = NULL;
}
